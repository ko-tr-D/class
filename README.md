...
PDF 원본은 Google Drive 서비스 계정 업로드 방식으로 준비했습니다. `GOOGLE_APPLICATION_CREDENTIALS`와 `GOOGLE_DRIVE_ROOT_FOLDER_ID`가 설정되어 있으면 PDF 업로드 시 Drive에 저장하고, 실패하거나 설정이 없으면 로컬 개발 저장소에 fallback 저장합니다.

## GitHub Pages

GitHub Pages 미리보기용 파일은 `docs/index.html`, `docs/app.js`, `docs/styles.css`에 복사되어 있습니다. 저장소 설정에서 Pages source를 `main` 브랜치의 `/docs` 폴더로 지정하면 아래 주소로 열 수 있습니다.
